...
class TweetsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, WriteTweetBackDelegate {
    ...
    @IBOutlet weak var composeBarButton: UIBarButtonItem! {
        didSet {
            let icon = UIImage(named: "Create-New-Filled")
    ...
    override func viewDidLoad() {
        ...
        navigationItem.rightBarButtonItem = UIBarButtonItem(image: UIImage(named: "Create-New-Filled"), landscapeImagePhone: nil, style: .done, target: self, action: #selector(composeTweet(_:)))
        ...
        let titleText = NSAttributedString(string: "Home", attributes: [
